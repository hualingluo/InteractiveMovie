...
export const AgentModal: React.FC<Props> = ({ onGenerate, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    // Fix: Pass 'late_shift' as the StyleMode to satisfy the type requirement and incorporate category/genre into the prompt
    const nodes = await generateStoryStructure(`${fullStyle} - ${theme}`, 'late_shift');
    if (nodes) {
      onGenerate(nodes);
      onClose();
    }
