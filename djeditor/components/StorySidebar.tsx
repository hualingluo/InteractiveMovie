    ...
    return (
        <div className="w-80 h-full bg-slate-900 border-r border-slate-700 flex flex-col z-20 shadow-xl">
            ...
            <div className="p-2 border-b border-slate-700 bg-slate-800/50 backdrop-blur">
                ...
                {/* Filters for Script Tab */}
                {tab === 'script' && (
                    <div className="flex gap-1 overflow-x-auto pb-1 no-scrollbar">
                        <button 
                            onClick={() => setFilterType('all')}
                            className={`px-2 py-1 rounded text-[10px] whitespace-nowrap transition-all border ${filterType === 'all' ? 'bg-slate-700 text-white border-slate-600' : 'bg-transparent text-slate-500 border-transparent hover:bg-slate-800'}`}
                        >
                            全部
                        </button>
                        <button 
                            onClick={() => setFilterType('scene')}
                            className={`px-2 py-1 rounded text-[10px] whitespace-nowrap transition-all border ${filterType === 'scene' ? 'bg-blue-600 text-white border-blue-500' : 'bg-transparent text-slate-500 border-transparent hover:bg-blue-900/20 hover:text-blue-400'}`}
                        >
                            剧情
                        </button>
                        <button 
                            onClick={() => setFilterType('decision')}
                            className={`px-2 py-1 rounded text-[10px] whitespace-nowrap transition-all border ${filterType === 'decision' ? 'bg-purple-600 text-white border-purple-500' : 'bg-transparent text-slate-500 border-transparent hover:bg-purple-900/20 hover:text-purple-400'}`}
                        >
                            选项
                        </button>
                        <button 
                            onClick={() => setFilterType('ending')}
                            className={`px-2 py-1 rounded text-[10px] whitespace-nowrap transition-all border ${filterType === 'ending' ? 'bg-red-600 text-white border-red-500' : 'bg-transparent text-slate-500 border-transparent hover:bg-red-900/20 hover:text-red-400'}`}
                        >
                            结局
                        </button>
                    </div>
                    ...
                    <>
                        {nodeList.length === 0 && (
                            <div className="text-center text-slate-500 text-xs py-10 flex flex-col items-center gap-2">
                                <Icons.FileText size={24} className="opacity-20" />
                                <span>未找到匹配节点</span>
                            </div>
                        )}
                        {nodeList.map(node => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-1">
                                    ...
                                    <span className={`text-[10px] px-1.5 py-0.5 rounded border flex-shrink-0 ${getTypeColor(node.type)}`}>
